...
<body>
  ...
  group('router adapter', () {
    test('descriptor maps to static, dynamic, or localized routes', () {
      SeoMetadata md(SeoParams _) =>
          const SeoMetadata(title: 't', description: 'd');
      SeoNode ct(SeoParams _, SeoHtml b) => b.h1('x');

      final static0 = SeoRouteDescriptor(
        path: '/pricing',
        metadata: md,
        content: ct,
      ).toSeoRoute();
      expect(static0.isDynamic, isFalse);

      final dynamic0 = SeoRouteDescriptor(
        path: '/post/[slug]',
        params: () async => [
      ...
      expect(dynamic0.isDynamic, isTrue);

      final localized = SeoRouteDescriptor(
        path: '/about',
        locales: const ['en', 'fr'],
        metadata: md,
        content: ct,
