...
seo_seed exists for one situation: you have a **Flutter web app** with routes
inside it that must be crawlable, and you want to keep a single Flutter codebase
rather than run a second stack.

If your goal is only to rank a handful of public pages (marketing, a blog, docs)
and those pages don't need the app's machinery, the honest advice is to **not
render them with Flutter at all**. Put them in static HTML, a static-site
generator, or a DOM-rendering Dart framework like [Jaspr](https://jaspr.site).
You get better Core Web Vitals (no engine to boot), real semantic HTML for free,
and no second representation to keep in sync — and it's what Flutter's own docs
recommend for text-rich content.

Reach for seo_seed when moving those pages off the canvas isn't worth it: the
content lives inside the app, shares its data and auth, and declaring per-route
crawler content beats maintaining a parallel site. It's a bridge for keeping one
Flutter codebase — not a substitute for a document-first web stack.
...
Your app imports only the runtime entry point — `package:seo_seed/runtime.dart`
— which pulls in none of the build-time code, so it compiles cleanly to web and
stays out of the app's dependency graph. `SeoRuntime.takeover()` is a no-op off
the web, so it's safe in shared `main` code. Your build tool imports the full
`package:seo_seed/seo_seed.dart`.
